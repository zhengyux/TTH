...
public class PhotoSelectorActivity extends Activity implements onItemClickListener, onPhotoItemCheckedListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        photoAdapter = new PhotoSelectorAdapter(getApplicationContext(), new ArrayList<PhotoModel>(),
                CommonUtils.getWidthPixels(this), this, this, this, images);
...
//        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CAMERA && resultCode == RESULT_OK) {
            if (data != null) {
                PhotoModel photoModel = new PhotoModel(CommonUtils.query(this, data.getData()));
                selected.clear();
                selected.add(photoModel);
                ok();
            } else {
                PhotoModel photoModel = new PhotoModel(CommonUtils.query(this, mPhotoUri));
                selected.clear();
                selected.add(photoModel);
                ok();
    ...
    public void onCheckedChanged(PhotoModel photoModel, CompoundButton buttonView, boolean isChecked) {
        if (PhotoSelectorActivity.size == 4 && isChecked) {
            return;
        }
        if (isChecked) {
            selected.add(photoModel);
