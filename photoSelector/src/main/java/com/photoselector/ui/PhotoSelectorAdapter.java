...
public class PhotoSelectorAdapter extends MBaseAdapter<PhotoModel> {
    ...
    public PhotoSelectorAdapter(Context context, ArrayList<PhotoModel> models, int screenWidth,
                                onPhotoItemCheckedListener listener, onItemClickListener mCallback, OnClickListener cameraListener, ArrayList<String> images) {
        this(context, models);
        this.images = images;
        ...
        if (position == 0 && CommonUtils.isNull(models.get(position).getOriginalPath())) { // 当时第一个时，显示按钮
            ...
            if (images != null) {
                for (String st : images) {
                    if (st.equals(models.get(position).getOriginalPath())) {
                        item.setSelected(true);
                    }
                }
            }
            item.cbPhoto.setClickable(true);
        }
        return convertView;
    }
}
